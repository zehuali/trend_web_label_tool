  ...
  if (Math.abs(zoomNS.zoomCumulativeDelta) > sensitivity) {
    if (zoomNS.zoomCumulativeDelta < 0) {
      if (centerPointer >= chartCenter) {
        if (minIndex <= 0) {
          maxIndex = Math.min(lastLabelIndex, maxIndex + 50);
        } else {
          minIndex = Math.max(0, minIndex - 50);
        }
      ...
      if (centerPointer >= chartCenter) {
        minIndex =
          minIndex < maxIndex
            ? (minIndex = Math.min(maxIndex, minIndex + 50))
            : minIndex;
      } else if (centerPointer < chartCenter) {
        maxIndex =
          maxIndex > minIndex
            ? (maxIndex = Math.max(minIndex, maxIndex - 50))
            : maxIndex;
      }
      zoomNS.zoomCumulativeDelta = 0;
    }
...
function doZoom(chartInstance, zoom, center, whichAxes) {
  ...
  if (
    zoomOptions &&
    helpers.getValueOrDefault(zoomOptions.enabled, defaultOptions.zoom.enabled)
  ) {
    // Do the zoom here
...
function panIndexScale(scale, delta, panOptions) {
  ...
  var offsetAmt = Math.max(
    scale.ticks.length - (scale.options.gridLines.offsetGridLines ? 0 : 1),
    1
  );
...
function doPan(chartInstance, deltaX, deltaY) {
  ...
  if (
    panOptions &&
    helpers.getValueOrDefault(panOptions.enabled, defaultOptions.pan.enabled)
  ) {
